...
const HUNTERS = {
  ...
  warlock: {
    ...
    basic:{ key:'LMB', name:'Affliction', emoji:'🟣', cd:0.55, kind:'proj', dmg:10, speed:600, range:580, radius:8, dot:{dps:14, dur:4} },
    q:{ key:'Q', name:'Plague', emoji:'☣️', cd:7, kind:'dotaoe', dmg:14, radius:175, delay:0.4, atCursor:true, dot:{dps:24, dur:5} },
    e:{ key:'E', name:'Shadowstep', emoji:'🌑', cd:7, kind:'blink', range:350 },
    r:{ key:'R', name:'Apocalypse', emoji:'☠️', cd:24, kind:'dotaoe', dmg:40, radius:300, delay:0.6, atCursor:true, dot:{dps:42, dur:6} }
  }
};
...
function abilityDesc(def){
  const d = def;
  switch (d.kind){
    case 'proj':  return `Fires ${d.pierce?'a piercing ':'a '}bolt for <b>${d.dmg}</b> damage.`+(d.dot?` Afflicts a DoT (<b>${d.dot.dps}</b>/s for ${d.dot.dur}s).`:'');
...
function setHunterHid(h,hid){ const d=HUNTERS[hid]; h.hid=hid; h.def=d; h.maxHp=d.maxHp; h.hp=d.maxHp; h.radius=d.radius; h.speed=d.speed*SPEED_SCALE; }
...
function fireProj(ent,def,ang,dmg){
  G.projectiles.push({ x:ent.x+Math.cos(ang)*ent.radius, y:ent.y+Math.sin(ang)*ent.radius,
    ...
    radius:def.radius, life:def.range/def.speed, pierce:!!def.pierce, color:ent.def.color, hits:new Set(), dot:def.dot||null });
}
...
function addAoe(ent,x,y,r,dmg,delay,dot){ G.aoes.push({x,y,r,dmg,team:ent.team,owner:ent,t:delay,max:delay,color:ent.def.color,dot:dot||null}); }
...
function clientApplySnapshot(snap){
  ...
  for (const a of snap.H){
    ...
    const fl=a[10];
    h.downed=!!(fl&1); h.hasCrown=!!(fl&2); h.snareStacks=(fl&4)?3:0; h.dots=(fl&8)?[1]:[];
  ...
  if (!G.barBuilt && G.player && G.player.def){ buildAbilityBar(); G.barBuilt=true; }
}
